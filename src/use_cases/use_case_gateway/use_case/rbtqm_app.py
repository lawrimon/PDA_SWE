...
try:
    ...
    for user in users:
        ...
        appointment_queue = channel.queue_declare(
            queue=f"appointment.{user}", durable=True
        )

        # Bind queues to appropriate exchanges
        channel.queue_bind(
            exchange="location", queue=location_queue.method.queue, routing_key=user
        )
        channel.queue_bind(
            exchange="event", queue=event_queue.method.queue, routing_key=user
        )
        channel.queue_bind(
            exchange="appointment",
            queue=appointment_queue.method.queue,
            routing_key=user,
        )
...
def callback(ch, method, properties, body):
    ...
    for event in data["events"]:
        ...
        if (
            abs(user_location["latitude"] - event_location["latitude"]) <= 0.1
            and abs(user_location["longitude"] - event_location["longitude"]) <= 0.1
        ):
...
if channel:
    ...
    for user in users:
        channel.basic_consume(
            queue=f"location.{user}", on_message_callback=callback, auto_ack=True
        )
        channel.basic_consume(
            queue=f"event.{user}", on_message_callback=callback, auto_ack=True
        )
        channel.basic_consume(
            queue=f"appointment.{user}", on_message_callback=callback, auto_ack=True
        )
