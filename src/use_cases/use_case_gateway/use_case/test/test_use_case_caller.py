...
def test_notify_event(mock_pika):
    ...
    assert (
        mock_pika.BlockingConnection.return_value.channel.return_value.exchange_declare.call_count
        == 4
    )
    assert (
        mock_pika.BlockingConnection.return_value.channel.return_value.queue_declare.call_count
        == 12
    )
    assert (
        mock_pika.BlockingConnection.return_value.channel.return_value.queue_bind.call_count
        == 12
    )
    assert (
        mock_pika.BlockingConnection.return_value.channel.return_value.basic_publish.call_count
        == 3
    )
