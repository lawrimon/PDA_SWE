...
@pytest.mark.parametrize(
    "user_id, data, expected_response, code_expected",
    [
        (
            "user1",
            {"language": "en", "stocks": "AAPL"},
            {"language": "en", "stocks": "AAPL"},
            200,
        ),
        (
            "user2",
            {"language": "fr", "spotify_link": "https://open.spotify.com/"},
            {"language": "fr", "spotify_link": "https://open.spotify.com/"},
            200,
        ),
        (
            "user3",
            {"invalidKey": "mypassword"},
            {"error": "Error getting preferences. No preferences found"},
            500,
        ),
        (
            "user4",
...
def test_manage_preferences(
    client, fake_redis, user_id, data, expected_response, code_expected
):
    # POST request to set preferences
    if user_id != "user4":
        response = client.post(f"/preferences/{user_id}", json=data)
